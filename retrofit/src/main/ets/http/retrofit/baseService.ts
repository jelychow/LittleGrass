...
import axios, { AxiosInstance, AxiosRequestConfig, AxiosResponse, FormData, InternalAxiosRequestConfig, ParamsSerializerOptions } from '@ohos/axios';
import { stringify } from '../third_parties/qs';
    ...
    if (this.__meta__[methodName].queryArrayFormat) {
      ...
      const options: ParamsSerializerOptions = {
        ...
        encode: (params: any): string => {
          //import qs  from '../third_parties/qs';
          return stringify(params);
        },
      };
      config.paramsSerializer = options
    }
...
export type RequestInterceptorFunction<T = any> =
(value: InternalAxiosRequestConfig<T>) => InternalAxiosRequestConfig<T> | Promise<InternalAxiosRequestConfig<T>>;
